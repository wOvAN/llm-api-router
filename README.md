...
- Each server can route through a proxy: `proxy` (empty = direct connection) with a `proxy_enabled` toggle (default on; `false` forces direct connection even with a proxy URL set). Supported schemes: `http`, `https`, `socks5`, `socks5h`, `socks4`, `socks4a`
